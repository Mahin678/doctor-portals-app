...
const DoctorBookingTime = ({ selectedDate }) => {
    ...
    return (
        <section>
            <h1 style={{ color: "#1cc7c1" }} className="text-center mb-5" >Available Appointment on {selectedDate.toDateString()}</h1>
            <div className="row" >
                {
                    bookingData.map(info =>
                        <BookingInfo key={info.id} selectedDate={selectedDate} info={info} />
                    )
                }
